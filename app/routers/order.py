...
from app.schemas import order_schema, users_schema

router = APIRouter()

# Crear un pedido
@router.post("/", response_model=order_schema.Order, status_code=status.HTTP_201_CREATED)
async def create_order(order: order_schema.OrderCreate, db: Session = Depends(database.get_db), current_user: users_schema.User = Depends(auth.get_current_user)):
...
async def read_order(order_id: int, db: Session = Depends(database.get_db), current_user: users_schema.User = Depends(auth.get_current_user)):
...
async def read_orders(db: Session = Depends(database.get_db), current_user: users_schema.User = Depends(auth.get_current_user)):
    if current_user.role not in ["local", "almacen"]:
        raise HTTPException(status_code=403, detail="No tienes suficientes permisos")
...
async def read_order_details(order_id: int, db: Session = Depends(database.get_db), current_user: users_schema.User = Depends(auth.get_current_user)):
    if current_user.role not in ["local", "almacen"]:
        raise HTTPException(status_code=403, detail="No tienes suficientes permisos")
